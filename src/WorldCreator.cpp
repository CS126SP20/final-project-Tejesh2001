...
void WorldCreator::CreateCeiling(b2World& world) const {
  ...
  if (app::getWindowWidth() == NULL) {
    ground_body_def.position.Set(conversions::ToBox2DCoordinates(800), -1.0);
  } else {
    ground_body_def.position.Set(conversions::ToBox2DCoordinates(
  ...
  if (app::getWindowWidth() == NULL) {
    ground_box.SetAsBox(conversions::ToBox2DCoordinates(800), 1);
  } else {
    ground_box.SetAsBox(conversions::ToBox2DCoordinates(
                            static_cast<float>(app::getWindowWidth())),
                        1);
  }
  // size the ground
...
void WorldCreator::CreateRightWall(b2World& world) const {
  ...
  float right_most_index = conversions::ToBox2DCoordinates(
      static_cast<float>(app::getWindowWidth()));
  float upper_most_index = conversions::ToBox2DCoordinates(
      static_cast<float>(app::getWindowHeight()) - 50);
